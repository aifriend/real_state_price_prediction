...
def get_merged_listings(parent):
    m_df = DataService.merge_dataset(
        load_listings(parent), load_listings_desc(parent), on='id')
...
def process_merged_listings(parent=0):
    # Load the data files into Python using libraries like pandas
    merged_df = get_merged_listings(parent)
